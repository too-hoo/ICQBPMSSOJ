<template>
  <div class="view">
    ...
    <Panel :title="contestId ? '比赛题目列表' : '题目列表'">
      <!--头部检索框,使用slot="header"以插槽的形式插入到头部右边-->
      <div slot="header">
        <el-input
          v-model="keyword"
      ...
      <div class="panel-options">
        ...
        <el-button v-if="contestId" type="primary"
                    ...
                    @click="addProblemDialogVisible = true">从公开的题目中添加
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 删除题目
      deleteProblem (id) {
        this.$confirm('你只能删除那些没有提交数据的题目,继续吗?', '删除题目', {
          type: 'warning'
        }).then(() => {
      ...
      makeContestProblemPublic (problemID) {
        this.$prompt('请输入显示ID(Display ID)来公开该题目', 'confirm').then(({value}) => {
    ...
    // 监听器:更新contestId,routeName,getProblemList
    watch: {
      '$route' (newVal, oldVal) {
